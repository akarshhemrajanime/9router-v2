...
async function refreshOAuthToken(connection) {
  ...
  try {
    ...
    if (provider === "qoder") {
      const response = await fetch("https://openapi.qoder.sh/api/v1/deviceToken/refresh", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "application/json", "User-Agent": "Go-http-client/2.0" },
  ...
  try {
    switch (connection.provider) {
      ...
      case "cb": {
        ...
        let valid = res.status !== 401 && res.status !== 403;
        let errorMessage = valid ? null : "Invalid CodeBuddy API key";

        if (res.status === 429) {
          const bodyText = await res.text().catch(() => "");
          if (bodyText.includes("trial version is not yet") || bodyText.includes("14017")) {
            valid = false;
            errorMessage = "Trial not activated (Login via Web first)";
          } else if (bodyText.includes("quota") || bodyText.includes("exhausted")) {
            valid = false;
...
export async function testSingleConnection(id) {
  ...
  if (result.balance !== undefined) {
    updateData.last_balance = result.balance;
  }
  ...
  if (result.refreshed && result.newTokens) {
    ...
    if (result.newTokens.cached_jwt) updateData.cached_jwt = result.newTokens.cached_jwt;
    if (result.newTokens.jwt_expires_at) updateData.jwt_expires_at = result.newTokens.jwt_expires_at;
